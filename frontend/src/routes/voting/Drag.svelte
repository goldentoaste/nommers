...
<div class="container-container">
  <div
    ...
    id="container"
    style="transform: translate({$coords.x + 0}px,{$coords.y +
      0}px) rotate({$coords.rot}deg) ;"
    on:mousedown={() => {
      dragging = true;
    }}
    on:mouseup={() => {
  ...
  >
    ...
    >
      <div
        style="transform: background-image: linear-gradient(
            180deg,
            rgba(0, 0, 0, 0) 32.81%,
            rgba(0, 0, 0, 0.9) 100%
          ),
          url({restaurantImg}); translate({170 * sign}px , {10 *
          sign}px)  rotate({-$coords.rot}deg) translate({-170 * sign}px , {-10 *
          sign}px);"
      >
